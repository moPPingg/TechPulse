...
def sentiment_lexicon(text: str) -> Tuple[float, str]:
    ...
    """
    score, method, pos_count, neg_count = sentiment_lexicon_extended(text)
    return score, method


def sentiment_lexicon_extended(text: str) -> Tuple[float, str, int, int]:
    """
    Returns (score, method, pos_count, neg_count). Score in [-1, 1]; method = 'lexicon'.
    """
    if not text or not isinstance(text, str):
        return 0.0, "lexicon", 0, 0
    t = text.lower()
    ...
    return max(-1.0, min(1.0, score)), "lexicon", pos_count, neg_count
