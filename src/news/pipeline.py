...
from src.news.sentiment import sentiment_lexicon
from src.news.ticker_align import align_article_to_tickers
...
def run_sentiment(conn, config: dict) -> int:
    ...
    for aid in ids:
        ...
        score, _ = sentiment_lexicon(text)
...
def run_pipeline(config_path: Optional[str] = None, steps: Optional[list] = None) -> dict:
    ...
    todo = steps or ["crawl", "clean", "sentiment", "align"]
    results = {}
    if "crawl" in todo:
