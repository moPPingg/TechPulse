"""
4. PatchTST
Patch-based Transformer: split series into patches, then Transformer encoder. PyTorch.
"""
...
try:
    ...
    class _PatchEmbedding(nn.Module):
        def __init__(self, seq_len: int, patch_len: int, n_channels: int, d_model: int):
            super().__init__()
            self.patch_len = patch_len
            self.n_patches = (seq_len - patch_len) // patch_len + 1
            self.proj = nn.Linear(patch_len * n_channels, d_model)
        def forward(self, x):
            B, T, C = x.shape
            p = self.patch_len
            patches = []
            for i in range(0, T - p + 1, p):
                patches.append(x[:, i : i + p, :].reshape(B, -1))
            x = torch.stack(patches, dim=1)
            return self.proj(x)
    class _PatchTST(nn.Module):
        def __init__(self, seq_len: int, n_channels: int, patch_len: int = 8, d_model: int = 64, n_heads: int = 4, n_layers: int = 2, dropout: float = 0.1):
            super().__init__()
            self.patch_len = patch_len
            self.patch_embed = _PatchEmbedding(seq_len, patch_len, n_channels, d_model)
...
class PatchTSTForecaster(BaseForecaster):
    """PatchTST: patch time series then Transformer. Input (n_samples, seq_len, n_features)."""
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, **kwargs) -> "PatchTSTForecaster":
        ...
        self.net = _PatchTST(seq_len, n_feat, self.patch_len, self.d_model, self.n_heads, self.n_layers).to(self.device)
